...
# swap to the KNN/vectorized population graph
# [NEW] swap to cell KNN 
from population_graph_KNN import Population_Graph_KNN as Population_Graph
from cell_graph_knn import Cell_Graph_KNN
...
class BiGraph_KNN(object):
    def __init__(
        ...
        sample_frac=1.0,
        k_cell_knn=20 # [NEW] KNN cell
    ):
        # [ORIGINAL] same parameters
        self.a = a
        self.n_iter = n_iter
        self.k_subtree_clustering = k_subtree_clustering
        self.k_patient_clustering = k_patient_clustering
        ...
        self.k_cell_knn = k_cell_knn # [NEW] KNN cell
    ...
    def analyze_survival(self, Patient_subgroups, survival_data, Patient_ids):
        ...
        for i in range(len(Patient_subgroups)):
            ...
            DF = pd.DataFrame(
                {"length": lengths, "status": statuses, "community": np.isin(Patient_ids, patient_ids)}
            )
            cph.fit(DF, duration_col="length", event_col="status", show_progress=False)
            ...
            Patient_subgroups[i]["hr_lower"] = np.exp(cph.confidence_intervals_["95% lower-bound"]["community"])
            Patient_subgroups[i]["hr_upper"] = np.exp(cph.confidence_intervals_["95% upper-bound"]["community"])
    ...
    def fit_transform(
        ...
        if os.path.exists(self.soft_wl_save_path + ".pkl"):
            print("There is a soft wl subtree kernel fitted before. We will load it directly from {}.pkl".format(self.soft_wl_save_path))
            print("If you want to re-fit soft wl subtree kernel, please delete the file '{}.pkl'".format(self.soft_wl_save_path))
            ...
            Num_cell_per_patient = [cell_graph[1].shape[0] for cell_graph in Cell_graphs]
            print("\t {} total cells, {:.2f} cells per patient.".format(sum(Num_cell_per_patient), sum(Num_cell_per_patient) / len(Patient_ids)))
            ...
            os.makedirs(os.path.dirname(self.soft_wl_save_path) or ".", exist_ok=True)
            ...
            print("\t {} unique patients.".format(len(singleCell_data["patientID"].unique())))
            print("\t {} unique cell types.".format(len(singleCell_data["celltypeID"].unique())))
            print("\t {} total cells, {:.2f} cells per patient.".format(len(singleCell_data), len(singleCell_data) / len(singleCell_data["patientID"].unique())))
            
            #  [NEW] --- v3: KNN cell graphs (remove old Cell_Graph usage) ---
            print(f"Start generating cell graphs (KNN at cell stage, k={self.k_cell_knn}).")
            cell_graph_ = Cell_Graph_KNN(a=self.a, k_cell_knn=self.k_cell_knn) # [NEW] KNN cell
            ...
            Similarity_matrix = soft_wl_subtree_.fit_transform(Cell_graphs, sample_frac=self.sample_frac)
        ...
        # === [NEW] only swap the Population_Graph implementation; prints preserved ===
        ...
        Num_patients_in_subgroups = [len(subgroup["patient_ids"]) for subgroup in Patient_subgroups]
        print("There are {} patient subgroups, {} ungrouped patients".format(
            len(Patient_subgroups), len(Patient_ids) - sum(Num_patients_in_subgroups))
        )

        print("Start finding characteristic patterns for each patient subgroup.")
        explainer_ = Explainer(threshold_hodges_lehmann=self.threshold_hodges_lehmann)
        ...
        if survival_data is not None:
            print("Since survival data is provided, we will sort patient subgroups by survival.\\But keep in mind, survival data is not touched in TME pattern discovery, patient subgroup detection, \\and characteristic pattern finding.")
            ...
            Patient_subgroups = self.analyze_survival(Patient_subgroups, survival_data, self.Patient_ids)
    ...
    def transform(
        ...
        # [ORIGINAL] unchanged — still uses Population_Graph (now KNN/vectorized)
        singleCell_data = singleCell_data.rename(
            columns={
                patientID_colname: "patientID",
                imageID_colname: "imageID",
        ...
        print("\t {} unique patients.".format(len(singleCell_data["patientID"].unique())))
        print("\t {} unique cell types.".format(len(singleCell_data["celltypeID"].unique())))
        print("\t {} total cells, {} cells per patient.".format(
            len(singleCell_data),
            len(singleCell_data) / len(singleCell_data["patientID"].unique()),
        ))

        #  [NEW] --- v3: use KNN cell graphs ---
        Cell_graphs = Cell_Graph_KNN(a=self.a, k_cell_knn=self.k_cell_knn).generate(singleCell_data)
        ...
        Population_graph_hat = population_graph_.generate(Similarity_to_new_data, Patient_ids_new)
        ...
        for i in range(len(self.Patient_subgroups)):
            ...
            Patient_subgroups_new.append({"subgroup_id": subgroup_id + "'", "patient_ids": patient_ids_new})
        if survival_data is not None:
            print("Since survival data is provided, we will analyze survival of each mapped subgroup. \\But keep in mind, survival data is not touched in subgroup mapping.")
            survival_data = survival_data.rename(
                columns={
                    patientID_colname: "patientID",
                    time_colname: "time",
            ...
            Patient_subgroups_new = self.analyze_survival(Patient_subgroups_new, survival_data, Patient_ids_new)
